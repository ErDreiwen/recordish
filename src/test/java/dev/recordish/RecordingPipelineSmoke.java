...
public final class RecordingPipelineSmoke {
    ...
    public static void main(String[] arguments) throws Exception {
        ...
        config.audioDelayPreset = RecordishConfig.AudioDelayPreset.NONE;
        ...
        if (!expectedAutoClipDirectory.equals(actualParent)
                || !output.getFileName().toString()
                        .startsWith("recordish-")) {
            throw new IllegalStateException(
                    "Auto-clip was not routed into its trigger folder: "
                            + output);
        }
        System.out.println("RECORDISH_PIPELINE_SMOKE_OK=" + output);
        ...
        System.out.println("RECORDISH_REPLAY_SMOKE_OK=" + replayOutput);
    }

    private static void verifyVideoFileClassification() {
        assertVideoName("recordish-final.mp4", true);
        assertVideoName("recordish-final.mkv", true);
        assertVideoName("recordish-final-recovered.mkv", true);
        assertVideoName("bedwars.part.1.mp4", true);
        assertVideoName("bedwars.partial.edit.mp4", true);
        assertVideoName("recordish-final.part.mp4", false);
        assertVideoName("recordish-final.video.mkv", false);
        assertVideoName("recordish-final.video-2.mkv", false);
        assertVideoName("recordish-final_compressed.part.mp4", false);
    ...
    private static Path runReplaySmoke(RecordishConfig config)
            throws Exception {
