...
public final class RecordingOverlay {
    ...
    private static void renderClassic(FontRenderer font, RecordishConfig config,
    ...
    private static void renderVhs(FontRenderer font, RecordishConfig config,
                                  RecordingManager manager, int width, int height) {
    ...
    private static void renderVhsPlayRec(FontRenderer font, RecordishConfig config,
                                         ThemeColors skin) {
        ...
        if (config.vhsShowPlay) {
            int playColor = RecordishConfig.applyOpacity(
                    skin != null
                            ? skin.textPrimary
                            : RecordishConfig.parseArgbColor(
                                    config.vhsPlayColor,
        ...
        int recTextColor = RecordishConfig.applyOpacity(
                skin != null
                        ? skin.textPrimary
                        : RecordishConfig.parseArgbColor(
                ...
                opacity);
        if (blinkOn()) {
            int dotColor = RecordishConfig.applyOpacity(
                    skin != null
                            ? skin.accent
                            : RecordishConfig.parseArgbColor(
    ...
    private static void renderVhsTimestamp(FontRenderer font, RecordishConfig config,
    ...
    private static void renderVhsDetails(FontRenderer font, RecordishConfig config,
                                         RecordingManager manager, int width, int height) {
        ...
        if (config.vhsShowDate) {
            int color = RecordishConfig.applyOpacity(
                    RecordishConfig.parseArgbColor(config.vhsDateColor, 0xFFFFFFFF),
                    opacity);
    ...
    private static void renderVhsPerformancePanel(FontRenderer font,
        ...
        for (int index = 0; index < lines.size(); index++) {
            ...
            drawText(font, lines.get(index), x + 2, y + index * 10,
                    RecordishConfig.applyOpacity(color, opacity));
        }
    }

    private static void renderSynthwave(FontRenderer font, RecordishConfig config,
                                        RecordingManager manager, int width, int height) {
        String line = "REC "
    ...
    private static void drawAudioMeter(int x, int y, int width, int height,
        ...
        int background = RecordishConfig.applyOpacity(0xFF333333, opacity);
        ...
        Gui.drawRect(x, y + barHeight + 1,
                ...
                RecordishConfig.applyOpacity(audioColor(rightLevel), opacity));
    }
    ...
    private static void drawBatteryIndicator(FontRenderer font, int x, int y,
        ...
        Gui.drawRect(x + 2, y + 2, x + 2 + fillWidth, y + height - 2,
                RecordishConfig.applyOpacity(audioColor(percent), opacity));
    ...
    private static int[] resolveClassicPosition(
        ...
        if (position == null) {
            position = RecordishConfig.OverlayPosition.TOP_LEFT;
        }
        switch (position) {
            case TOP_RIGHT:
