...
public final class ScreenCapture {
    ...
    public synchronized void prepare(int width, int height, int pooledFrames) {
        ...
        pool = new FrameBufferPool(
            outputByteSize,
            pooledFrames,
            RecordishConfig.get().frameBufferPoolingEnabled);
        ...
        if (pboApi == PBO_API_UNDETECTED) {
            ...
            if (pboApi == PBO_API_NONE) {
                RecordishMod.LOGGER.info(
                    "Pixel-buffer objects are unavailable; Recordish will use synchronous framebuffer reads.");
            }
        }
    ...
    private void deletePbosSafely() {
        ...
        try {
            ...
            RecordishMod.LOGGER.debug("Failed to delete screen-capture PBOs.", throwable);
            pboIds[0] = 0;
            pboIds[1] = 0;
        } finally {
            /*
