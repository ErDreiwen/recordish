...
import dev.recordish.theme.ThemedSlider;
...
public final class ExportSettingsScreen extends GuiScreen {
    ...
    private static String getFormatDisplay(RecordishConfig config) {
        return isEmpty(config.exportFormat)
                ? "Auto"
                : config.exportFormat.toUpperCase(Locale.ROOT);
    }

    private static String getVideoCodecDisplay(RecordishConfig config) {
        return isEmpty(config.exportVideoCodec)
                ? "Auto"
                : config.exportVideoCodec.toUpperCase(Locale.ROOT);
    }

    private static String getAudioCodecDisplay(RecordishConfig config) {
        return isEmpty(config.exportAudioCodec)
                ? "Auto"
                : config.exportAudioCodec.toUpperCase(Locale.ROOT);
    }

    private static String getResolutionDisplay(RecordishConfig config) {
        return isEmpty(config.exportResolution)
                ? "Recording"
                : config.exportResolution;
    }

    private static String getFpsDisplay(RecordishConfig config) {
