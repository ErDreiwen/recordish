 ...
 * crisp at every GUI scale and lets the existing Recordish theme presets
 * remain visible on the legacy client.</p>
 */
public class RecordishButton extends GuiButton {
    public RecordishButton(
            int buttonId,
            int x,
            int y,
            int width,
    ...
    public void drawButton(Minecraft minecraft, int mouseX, int mouseY) {
        ...
        if (RecordishConfig.get().uiScanlines && height >= 16) {
    ...
    private static ThemeColors colors() {
        try {
            return ThemeColors.forPreset(RecordishConfig.get().uiTheme);
