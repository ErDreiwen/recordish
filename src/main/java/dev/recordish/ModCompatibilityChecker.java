...
public final class ModCompatibilityChecker {
    ...
    static {
        ...
        conflicts.put(
            "replay-mod",
            "Replay Mod may conflict with Recordish's capture hooks.");
        conflicts.put(
            "bettershields",
    ...
    public static synchronized List<String> checkAndLog() {
        ...
        for (Map.Entry<String, String> entry
            ...
            try {
                if (Loader.isModLoaded(entry.getKey())) {
                    ...
                    RecordishMod.LOGGER.warn("[Recordish] {}", message);
        ...
        if (conflicts.isEmpty()) {
            RecordishMod.LOGGER.info(
                "[Recordish] No known mod conflicts detected.");
        } else {
            RecordishMod.LOGGER.warn(
                "[Recordish] {} known conflicting mod(s) detected.",
    ...
    public static void warnPlayerIfConflicts() {
        if (!hasConflicts()
                || !RecordishConfig.get().notifyWarnings) {
            return;
        }
        boolean bridge = RecordishConfig.get().replayCompatBridge;
        ...
        if (replayCoexisting) {
            RecordishMessages.send(
                ...
                "\u00A7eRecordish compatibility bridge is active for "
