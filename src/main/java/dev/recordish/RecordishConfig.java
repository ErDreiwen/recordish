...
public final class RecordishConfig {
    ...
    public static synchronized void initialize(File minecraftDirectory, File forgeConfigDirectory) {
        ...
        configPath = forgeConfigDirectory.toPath().resolve("recordish.json");
        legacyConfigPath = forgeConfigDirectory.toPath().resolve("recordable.json");
        instance = loadInternal();
    }

    public static RecordishConfig get() {
        if (instance == null) {
    ...
    private static RecordishConfig loadInternal() {
        RecordishConfig loaded = null;
        Path sourcePath = configPath;
        boolean importingLegacyConfig = false;
        if (!Files.isRegularFile(sourcePath)
                && Files.isRegularFile(legacyConfigPath)) {
            sourcePath = legacyConfigPath;
            importingLegacyConfig = true;
        }
        if (Files.isRegularFile(sourcePath)) {
            try (Reader reader = Files.newBufferedReader(
                    sourcePath,
                    StandardCharsets.UTF_8)) {
                loaded = GSON.fromJson(reader, RecordishConfig.class);
            } catch (Exception exception) {
                RecordishMod.LOGGER.warn(
                        "Unable to read Recordish config from {}; "
        ...
        if (loaded == null) {
            loaded = new RecordishConfig();
        } else if (importingLegacyConfig) {
            RecordishMod.LOGGER.info(
                    "Imported legacy Record-able settings from {} into {}.",
                    legacyConfigPath,
                    configPath);
        }
        loaded.sanitize();
    ...
    public synchronized void save() {
        ...
        try {
            ...
            RecordishMod.LOGGER.error("Unable to save Recordish config.", exception);
        }
    }
