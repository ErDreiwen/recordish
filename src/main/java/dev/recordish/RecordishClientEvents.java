...
import dev.recordish.screen.FfmpegWelcomeScreen;
import dev.recordish.screen.RecordishHomeButton;
import dev.recordish.screen.VideoCollectionScreen;
 ...
 */
public final class RecordishClientEvents {
    private static final String CATEGORY =
            "key.categories.recordish.main";
    ...
    private static final RecordishClientEvents INSTANCE =
            new RecordishClientEvents();
    ...
    private RecordishHomeButton homeButton;
    private GuiScreen initializedMainMenu;

    private RecordishClientEvents() {
    }
    ...
    private void registerKeybindings() {
        ...
        toggleRecording = register(
                "key.recordish.toggle_recording",
                resolveHotkeyDefault(
                    config.hotkeyToggleRecording,
                    RecordishConfig.DEFAULT_HOTKEY_TOGGLE_RECORDING,
                    claimedKeys));
        pauseResume = register(
                "key.recordish.pause_resume",
                resolveHotkeyDefault(
                    config.hotkeyPauseResume,
                    RecordishConfig.DEFAULT_HOTKEY_PAUSE_RESUME,
                    claimedKeys));
        openSettings = register(
                "key.recordish.open_settings",
                resolveHotkeyDefault(
                    config.hotkeyOpenSettings,
                    RecordishConfig.DEFAULT_HOTKEY_OPEN_SETTINGS,
                    claimedKeys));
        openVideos = register(
                "key.recordish.open_video_collection",
                resolveHotkeyDefault(
                    config.hotkeyOpenVideoCollection,
                    RecordishConfig.DEFAULT_HOTKEY_OPEN_VIDEO_COLLECTION,
                    claimedKeys));
        addBookmark = register(
                "key.recordish.add_bookmark",
                config.hotkeyAddBookmark);
        pushToTalk = register(
                "key.recordish.push_to_talk",
                resolveHotkeyDefault(
                    config.hotkeyPushToTalk,
                    RecordishConfig.DEFAULT_HOTKEY_PUSH_TO_TALK,
                    claimedKeys));
        saveReplay = register(
                "key.recordish.save_replay_buffer",
                config.hotkeySaveReplayBuffer);
        toggleCensor = register(
                "key.recordish.toggle_censor_overlay",
                config.hotkeyToggleCensorOverlay);
        openCensorEditor = register(
                "key.recordish.open_censor_editor",
    ...
    public void onClientTick(TickEvent.ClientTickEvent event) {
        ...
        while (openSettings.isPressed()) {
            openScreen(
                    "dev.recordish.screen.RecordishSettingsScreen",
                    minecraft.currentScreen);
        }
        while (openVideos.isPressed()) {
            if (!openScreen(
                    "dev.recordish.screen.VideoCollectionScreen",
                    minecraft.currentScreen)) {
                PlatformUtils.open(
        ...
        while (openCensorEditor.isPressed()) {
            openScreen(
                    "dev.recordish.screen.CensorOverlayEditorScreen",
                    minecraft.currentScreen);
        }
    ...
    public void onMainMenuInitialized(
        ...
        if (config.showHomeButton) {
            ...
            homeButton = new RecordishHomeButton(
                    HOME_BUTTON_ID,
                    x,
                    y);
    ...
    private static boolean openScreen(
        ...
        try {
            ...
            RecordishMessages.send(
                    ChatCategory.GENERAL,
                    "That Recordish screen is not available yet.");
            return false;
        } catch (Throwable throwable) {
            RecordishMod.LOGGER.warn(
                    "Unable to open Recordish screen {}.",
                    className,
                    throwable);
            return false;
        }
    ...
    private static boolean invokeReplaySave() {
        try {
            Class<?> replayClass =
                    Class.forName("dev.recordish.ReplayBuffer");
