...
public final class ReplayBuffer {
    ...
    public void start(
        ...
        start(
                ...
                fps,
                durationSeconds,
                RecordishConfig.get().replayBufferQuality);
    }
    ...
    public void start(
        ...
        synchronized (lock) {
            ...
            try {
                Path outputDirectory = RecordishConfig.get()
                        .getOutputDirectory();
                bufferDirectory = RecordishPaths.preferredDirectory(
                        outputDirectory,
                        ".recordish-replay-buffer",
                        ".recordable-replay-buffer")
                        .toAbsolutePath()
                        .normalize();
        ...
        if (retainedSeconds < seconds) {
            ...
            RecordishMessages.send(
                    ...
                    warning);
            RecordishMod.LOGGER.warn(warning);
        }
    }
    ...
    private Path preserveRecoveryDirectoryLocked(
        ...
        try {
            Files.write(
                    ...
                    ("Recordish replay save was interrupted during JVM "
    ...
    private static void clearBufferDirectory(Path directory)
        ...
        if (!".recordish-replay-buffer".equals(directoryName)
                && !".recordable-replay-buffer".equals(directoryName)) {
            throw new IOException(
    ...
    private static Path uniqueRecoveryDirectory(Path parent)
        ...
        String base = ".recordish-replay-recovery-"
    ...
    static int[] resolveStoredDimensions(
        ...
        int safeWidth = makeEven(Math.max(2, width));
        int safeHeight = makeEven(Math.max(2, height));
        int[] preset = RecordishConfig.resolveReplayPreset(
                quality,
                safeHeight,
                MAX_REPLAY_FPS);
    ...
    private static void notifyClient(
        ...
        minecraft.addScheduledTask(new Runnable() {
            @Override
            public void run() {
                RecordishMessages.send(category, message);
            }
        });
    }
