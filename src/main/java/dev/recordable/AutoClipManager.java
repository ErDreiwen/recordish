...
public final class AutoClipManager {
    ...
    private void checkDeath(EntityPlayer player, RecordableConfig config) {
    ...
    private void checkDimension(EntityPlayer player, RecordableConfig config) {
    ...
    private void checkTrackedKill(RecordableConfig config) {
    ...
    private void updateKillTracking(RecordableConfig config) {
        if (config.autoClipOnKill || config.autoClipOnPlayerKill) {
            checkTrackedKill(config);
        } else {
            clearKillTracking();
    ...
    private void triggerKillClip(RecordableConfig config, String reason) {
    ...
    private void scheduleMontage(RecordableConfig config, String reason) {
        if (montageSaveTicks >= 0) {
            RecordableMod.LOGGER.debug(
    ...
    private void triggerForwardClip(RecordableConfig config, String reason) {
        if (!acquireCooldown(reason)) {
            return;
        }
        if (ownsForwardRecording) {
