...
public final class PerformanceOptimizer {
    ...
    public synchronized boolean applyPendingRecommendation() {
        ...
        return applyRecommendation(pending, RecordableConfig.get());
    }
    ...
    private static boolean lowerQuality(RecordableConfig config) {
    ...
    private static boolean lowerFps(RecordableConfig config) {
        if (config.fps > 60) {
            config.fps = 60;
            return true;
        }
    ...
    private static boolean lowerResolution(RecordableConfig config) {
